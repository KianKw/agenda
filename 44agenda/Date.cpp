...
Date::Date(const std::string &dateString) {
    ...
    std::string::size_type arr[] = {0,1,2,3, 5,6, 8,9, 11,12, 14,15};
    for (std::string::size_type i = 0; i < 12; i++) {
...
bool Date::isValid(const Date &t_date) {
    if (!(0 <= t_date.m_minute && t_date.m_minute <= 59)) {
        ...
        return false;
    } else if (!(1 <= t_date.m_month && t_date.m_month <= 12)) {
        return false;
    } else if (!(1000 <= t_date.m_year && t_date.m_year <= 9999)) {
        return false;
    } else {
        if ((t_date.m_month == 1)
          ||(t_date.m_month == 3)
          ...
          ||(t_date.m_month == 10)
          ||(t_date.m_month == 12)) {
            if (!(1 <= t_date.m_day && t_date.m_day <= 31))
                return false;
        } else if ((t_date.m_month == 4)
                   ||(t_date.m_month == 6)
                   ||(t_date.m_month == 9)
                   ||(t_date.m_month == 11)) {
            if (!(1 <= t_date.m_day && t_date.m_day <= 30))
                return false;
        } else {
            if (IsLeapYear(t_date.m_year)) {
                if (!(1 <= t_date.m_day && t_date.m_day <= 29))
                    return false;
            } else {
                if (!(1 <= t_date.m_day && t_date.m_day <= 28))
                    return false;
            }
        }
    }
...
std::string Date::dateToString(const Date &t_date) {
    std::string temp = "0000-00-00/00:00";
    if (isValid(t_date)) {
        temp = "";
