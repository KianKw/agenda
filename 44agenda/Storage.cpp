...
Meeting StringToMeeting(std::string meeting_str) {
    ...
    for (std::string::size_type j = 1; j < buf.size(); ++j) {
        if (buf.at(j) == '&') {
...
bool Storage::readFromFile(void) {
    std::cout << "bool Storage::readFromFile(void)" << std::endl;
...
bool Storage::writeToFile(void) {
    ...
    user_write.close();
    std::ofstream meeting_write(Path::meetingPath, std::ios::out|std::ios::trunc);
